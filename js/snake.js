...
var MARGE = {'x':10, 'y':10};
var FENETRE = {'x':window.innerWidth, 'y':window.innerHeight};
var GRILLE = {'x':Math.floor(FENETRE.x/TAILLE.x),'y':Math.floor(FENETRE.y/TAILLE.y)};
var TOUR_FENETRE = {'x':(FENETRE.x-GRILLE.x*TAILLE.x)/2, 'y':(FENETRE.y-GRILLE.y*TAILLE.y)/2}
const COLOR = {'serpent':'green', 'pomme':'red'};
...
function setDimensionCase(nbCaseX=9, nbCaseY=null) {
    if (nbCaseX === null && nbCaseY !== null){
        TAILLE = {
            x: FENETRE.y/nbCaseY,
            y: FENETRE.y/nbCaseY
        };
...
function setDimensionMarge(pourcentageX=0.1, pourcentageY=null) {
    if (pourcentageX == null && pourcentageY != null){
        MARGE = {
            x: TAILLE.x*pourcentageY,
            y: TAILLE.y*pourcentageY
        };
    }
    if (pourcentageX != null && pourcentageY == null){
        MARGE = {
            x: TAILLE.x*pourcentageX,
            y: TAILLE.y*pourcentageX
        };
...
function updateGrille(){
    GRILLE = {'x':Math.floor(FENETRE.x/TAILLE.x),'y':Math.floor(FENETRE.y/TAILLE.y)};
}
function updateTourFenetre(){
    TOUR_FENETRE = {'x':(FENETRE.x-GRILLE.x*TAILLE.x)/2, 'y':(FENETRE.y-GRILLE.y*TAILLE.y)/2};
}
